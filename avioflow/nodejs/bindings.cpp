...
public:
    ...
    AudioDecoderAddon(const Napi::CallbackInfo& info) 
        : Napi::ObjectWrap<AudioDecoderAddon>(info), decoder(std::make_unique<avioflow::AudioDecoder>()) {}
...
private:
    ...
    Napi::Value DecodeNext(const Napi::CallbackInfo& info) {
        ...
        if (samples.data.empty()) return info.Env().Null();
        ...
        for (size_t c = 0; c < samples.data.size(); ++c) {
            Napi::Float32Array data = Napi::Float32Array::New(info.Env(), samples.data[c].size());
